...
import { applyStrangerMutation, emptyStrangerExperiment, summarizeStrangerExperiment } from "./stranger-experiment";

test("읽음 여부별 열람, 작성, 제출을 고유 참여자로 집계하고 재전송을 중복하지 않는다", () => {
  let data = emptyStrangerExperiment();
  data = applyStrangerMutation(data, { type: "visit", userId: "a", nickname: "A" });
  data = applyStrangerMutation(data, { type: "visit", userId: "a", nickname: "A" });
  data = applyStrangerMutation(data, { type: "visit", userId: "b", nickname: "B" });
