...
Figma 닉네임 설정 다이얼로그에 `0/10`과 `2~10자`가 적혀 있다.
...
만든다. 길이 검사를 화면에서도 해야 하므로 상수를 `companion object`에 공개해 둔다.
...
일괄 상태 변경에서 여러 권의 기록이 한 번에 지워질 수 있다. 확인 다이얼로그가 필요한지는
[미결정 사항](#11-미결정-사항)에 남겼다.
...
**정체성**
25. 닉네임이 2자 미만이거나 10자 초과면 만들 수 없다.
...
| 일괄 상태 변경 확인 | 미정 | 여러 권을 「읽고 싶어요」로 되돌릴 때 진행 기록 소실을 경고할지 |
